<x-layouts.admin :title="__('Admin · New role')" admin-key="roles" page="roles-create">
    ...
    <div class="px-4 sm:px-6 lg:px-7 pt-7 pb-2">
        <div>
            <div class="font-mono text-[10px] uppercase tracking-[0.18em] text-ink-500 mb-2">
                {{ __('Admin · Roles · New') }}</div>
            <h1 class="font-serif font-normal tracking-[-0.01em] text-[26px] sm:text-[30px] lg:text-[36px] leading-[1.0]">{{ __('Create a new') }}
                <span class="italic text-wa-deep">{{ __('role') }}</span></h1>
            <p class="text-[13px] text-ink-600 mt-2 max-w-2xl">
                {{ __('Pick a name, then check the boxes to grant this role access to specific modules. You can edit this any time.') }}
            </p>
        </div>
    </div>
    ...
    <main class="px-4 sm:px-6 lg:px-7 pb-7">
        ...
        <form id="roleForm" method="POST" action="{{ route('admin.roles.store') }}" class="space-y-5">
            @csrf
            ...
            <div class="bg-white border border-paper-200 rounded-[14px] shadow-card p-5">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
                    <div>
                        ...
                        <input id="role-name" name="name" type="text" value="{{ old('name') }}"
                            ...
                            placeholder="{{ __('e.g. Sales Lead') }}" required>
                    </div>
                    <div>
                        <label
                            class="text-[11.5px] font-semibold text-ink-700 flex items-center justify-between gap-2 mb-[5px]"
                                ...
                                <td class="px-3 py-3 align-top">
                                    <div class="flex flex-wrap gap-x-4 gap-y-1.5">
                                        @foreach ($perms as $perm)
                                            @php $checked = in_array($perm->name, old('permissions', []), true); @endphp
    ...
    @push('scripts')
        <script>
            // Live count + module + grand select-all bindings.
            (function() {
                ...
                table.querySelectorAll('tr').forEach(row => {
                    ...
                    perms.forEach(p => p.addEventListener('change', refresh));
                });
