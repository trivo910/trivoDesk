...
class UsersController extends Controller
{
    ...
    public function create(): View
    {
        return view('admin.users.create', [
            ...
            'platformRoles'  => Role::query()->orderBy('name')->get(['id', 'name']),
        ]);
    }
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        $roleNotice = $this->syncPlatformRole($request, $user);
        ...
        $msg = 'User created.';
        if ($roleNotice) $msg .= ' ' . $roleNotice;
        if ($mailNotice) $msg .= ' Welcome email skipped: ' . $mailNotice;
        return redirect()->route('admin.users.edit', $user->id)->with('success', $msg);
    }
    ...
    public function edit(string $id): View
    {
        ...
        return view('admin.users.edit', [
            ...
            'roles'           => $this->roleOptions(),
            'platformRoles'   => Role::query()->orderBy('name')->get(['id', 'name']),
            'userPlatformRoleId' => $user->roles->first()?->id,
        ]);
    }
    ...
    /**
     * Assign the submitted Spatie platform role (custom roles created under
     * /admin/roles, plus the built-in Super Admin / Platform Support /
     * Auditor) to the user. This is separate from the `role` column select
     * above — that one is the coarse admin/owner/user/agent tier, this is
     * the fine-grained permission bundle.
     *
     * Returns null on success/no-op, or a notice string if the change was
     * blocked (e.g. non-Super-Admin trying to grant Super Admin).
     */
    private function syncPlatformRole(Request $request, User $user): ?string
    {
        if (!$request->has('platform_role_id')) {
            return null;
        }

        $roleId = $request->input('platform_role_id');
        $newRole = $roleId !== '' && $roleId !== null
            ? Role::find($roleId)
            : null;
        ...
        $touchesSuperAdmin = $newRole?->name === PlatformPermissions::ROLE_SUPER_ADMIN
            || $currentRole?->name === PlatformPermissions::ROLE_SUPER_ADMIN;
        if ($touchesSuperAdmin && !PlatformPermissions::isSuperAdmin($request->user())) {
            Audit::log('admin.user.platform_role_change_denied', [
                'resource' => $user,
                'result'   => 'failure',
                'meta'     => ['attempted_role' => $newRole?->name, 'from' => $currentRole?->name, 'reason' => 'requires_super_admin'],
            ]);
            return 'Granting/revoking Super Admin requires Super Admin — platform role unchanged.';
        }

        $user->syncRoles($newRole ? [$newRole] : []);
        Audit::log('admin.user.platform_role_changed', [
            'resource' => $user,
            'meta'     => ['from' => $currentRole?->name, 'to' => $newRole?->name],
        ]);
        return null;
    }
